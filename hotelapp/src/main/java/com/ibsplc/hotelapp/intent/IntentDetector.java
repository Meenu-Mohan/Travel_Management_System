    ...
    private static void extractBookingDetails(String message, Intent intent) {
      ...
      // Extract hotel name (capture until 'room', 'in', 'from', or end, but stop before 'room')
        ...
        if (hotelMatcher.find()) {
            ...
            // Ensure hotel name does not include 'room' or subsequent words
            int roomIndex = hotelName.toLowerCase().indexOf(" room ");
            if (roomIndex != -1) {
                hotelName = hotelName.substring(0, roomIndex).trim();
            }
